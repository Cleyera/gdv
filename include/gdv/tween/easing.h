#ifndef GDV_EASING_H_
#define GDV_EASING_H_
...
Ty in_sine(Ty begin, Ty end, Ty ratio) noexcept {
    return (begin - end) * ::std::cos(::gdv::pi<Ty> * ratio) + end;
}

template <typename Ty>
Ty out_sine(Ty begin, Ty end, Ty ratio) noexcept {
    return (end - begin) * ::std::sin(::gdv::pi<Ty> * ratio) + begin;
}

template <typename Ty>
Ty in_out_sine(Ty begin, Ty end, Ty ratio) noexcept {
    return (begin - end) / static_cast<Ty>(2) * (::std::cos(::gdv::pi<Ty> * ratio) - static_cast<Ty>(1)) + begin;
}
...
Ty in_circ(Ty begin, Ty end, Ty ratio) noexcept {
    return begin - (end - begin) * (::std::sqrt(static_cast<Ty>(1) - ratio * ratio) - static_cast<Ty>(1));
}
